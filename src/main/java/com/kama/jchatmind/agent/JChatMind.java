...
public class JChatMind {
    ...
    private boolean think(int stepIndex) {
        ...
        try {
            String thinkPrompt = """
                    现在你是一个智能体的「决策模块」。
                    请根据当前对话上下文，决定下一步的动作（调用工具 / 直接回答 / 结束任务）。

                    【任务执行原则】
                    1. 识别出用户最近一条消息里的**所有子任务**（例如"查询并邮件发送"= 2 个子任务）；
                    2. 每轮只调用一个工具来推进**下一个还没完成的子任务**；
                    3. **所有子任务都完成后**，给出面向用户的最终答复并调用 terminate 结束；
                    4. 不要臆造信息，包括表名、邮箱地址等——用户给了什么就用什么，没给就向用户索要；
                    5. 只有当"从自然语言上下文无法获得所需信息"时，才去调用 KnowledgeTool 检索知识库。
                       业务表的元数据、工具使用细节等属于系统已知信息，不要查知识库。
